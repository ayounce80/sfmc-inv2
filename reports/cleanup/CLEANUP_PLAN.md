...
**Cleanup Candidates:** 1,047 objects
...
- **555 HIGH confidence** cleanup candidates (safe to delete)
- **492 MEDIUM confidence** candidates (review recommended)
- **176 LOW confidence** candidates (business owner approval needed)
- **46 Journey** cleanup candidates (28 Draft, 18 Stopped)

### Extraction Improvements

This analysis uses enhanced multi-BU extraction that captures:
- Automations from all 3 Business Units (Parent, DT, AT)
- 10,694 relationship mappings (import→DE, query→DE, filter→DE)
- 90-day staleness threshold for automations

**Result:** 210 fewer false-positive orphans compared to previous single-BU analysis.
...
### TEST Data Extensions (84 objects)
...
### BACKUP/ARCHIVE Data Extensions (13 objects)
...
**Paused & Stale (48):**
- Automations paused and not run in 90+ days

**Ready but Very Old (187):**
- Status "Ready" but last run over 1 year ago
- Includes automations from 2014-2024

**Ready but Stale (21 MEDIUM):**
- Status "Ready" but not run in 90+ days
...
### Very Old Data Extensions (368 objects)
...
### Old Data Extensions (176 objects, LOW confidence)

DEs not modified in 1-3 years but not matching TEST/BACKUP patterns.

### Other Orphaned Objects (96 DEs + 79 other = 175 REVIEW)
...
1. **`reports/cleanup/orphaned_data_extensions.csv`** - 737 orphaned DEs with confidence ratings
2. **`reports/cleanup/stale_automations.csv`** - 277 automation cleanup candidates
3. **`reports/cleanup/other_orphans.csv`** - 384 other orphaned objects
4. **`reports/cleanup/SFMC_Cleanup_Analysis.xlsx`** - Excel workbook with all reports
...
2. Delete TEST DEs via API (84 objects)
3. Delete BACKUP/ARCHIVE DEs (13 objects)
...
This cleanup analysis uses:
- **Multi-BU extraction** across Parent (1045947), DT (1054904), and AT (1067662)
- **10,694 relationship mappings** including:
  - automation_contains_* (query, import, extract, filter, etc.)
  - query_writes_de, query_reads_de
  - import_writes_de, filter_writes_de
  - journey_uses_* (email, event, DE, etc.)
  - triggered_send_uses_* (email, list, classification)
- **90-day staleness threshold** for automation activity
- **Dynamic date calculations** (1 year = OLD, 3 years = VERY_OLD)
